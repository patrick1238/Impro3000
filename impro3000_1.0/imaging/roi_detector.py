...
def detect_roi(image,dimension=None,roiThreshold=None,identifier="ROI"):
    """
    ...
    """
    scalingfactor = (round(np.average(image))/167)
    if dimension == None:
        dimension = max(int(1000*((2-scalingfactor)**2)),1)
    if roiThreshold == None:
        roiThreshold = min(int(200 *math.sqrt(scalingfactor)),240)
