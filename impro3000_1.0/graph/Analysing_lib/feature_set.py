...
class feature_set:
    ...
    def save(self,path):
        output_file =  open(path + self.__cellGraph.get_identifier() + "_analysis.csv", "w")
        output_file.write(self.__create_output_string())
        output_file.close()
            
        
    def __create_output_string(self):
        header = "identifier,diagnose"
        values = self.__cellGraph.get_identifier()+","+self.__cellGraph.get_diagnose()
        for key in self.__feature_set:
            header = header + "," + key
