...
class WSI():
    ...
    def get_RGB_numpy_array(self, x_coord=0, y_coord=0, width=-1, height=-1, layer=0):
        ...
        new_layer = min(layer,self.get_layer_count()-1)
        if height == -1:
            height = self.get_height(new_layer)
        if width == -1:
            width = self.get_width(new_layer)
        image_reader = bf.ImageReader(path=self.input_path, perform_init=True) # what does the init do? no idea.
        tmp_nparray = image_reader.read(series=new_layer, XYWH=(x_coord, y_coord, width, height))
        image_reader.close()
