...
from preanalysis import analyse
...
from reader.std_pckgs.basic_functions import prepare_workspace, prepare_batch_workspace
...
def start_thread(image_name, diagnose, stain, arguments):
    cellobject_thread = post_thread.Postprocessing_thread(image_name,diagnose,stain,arguments["cellobjects"])
...
def preanalysis(image_file,config):
    analyse(image_file,config)
...
def analyse_image(image_file,config):
    ...
    if config.get("postprocessing_active")=="active":
        cellobject_thread = start_thread(wsi.get_image_name(),wsi.get_diagnose(),config.get("staining"),arguments)
    if config.get("get_full_image") == True:
        ...
        layer3_image = wsi.get_RGB_numpy_array(layer=3)
        roi = roi_detector.detect_roi(layer3_image)
        layer3_image=None
...
if __name__ == '__main__':
    ...
    prepare_batch_workspace(config)
    if config.get("preanalysis_active")=="active":
        if os.path.isdir(in_path):
            for file in os.listdir(in_path):
                file = in_path+file
                if os.path.isfile(file):
                    preanalysis(file,config)
        if os.path.isfile(in_path):
            preanalysis(in_path,config)
