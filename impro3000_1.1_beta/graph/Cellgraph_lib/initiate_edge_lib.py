...
def __get_distance(point1,point2,dimension=2):
    if dimension == 2:
...
def __get_cellnumber_inside_communicationarea(nodes,point1,point2,binned,communication_threshold):
    binsize = int(round(1.5*communication_threshold))
    counter = 0
    area = __get_communication_area(point1,point2)
    nodesToBeAnalysed = binning_lib.getCommunicationAreaBins(point1,point2,area,binned,binsize)
...
def __analyse_bin(cellgraph,binned,y,perpendicular,extended,communication_threshold,dimension):
    ...
    for horitzontal in perpendicular:
        ...
        for key in horitzontal:
            for key2 in coordinates:
                if key != key2:
                    distance = __get_distance(nodes[key],nodes[key2],dimension)
                    if distance < communication_threshold:
                        ...
                        if np.random.sample() <= commmunicationProbability:
                            cellgraph.add_edge(key,key2,distance)
        x += 1

def initialize_deterministic_graph(cellgraph, communication_threshold,dimension):
    binsize = int(round(1.5*communication_threshold))
    binned = binning_lib.bin_cellgraph(cellgraph,binsize)
    nodes = cellgraph.get_nodes("position")
    x = 0
    y = 0
...
def initialize_waxmann_graph(cellgraph,communication_threshold,dimension):
    binsize = int(round(1.5*communication_threshold))
    binned = binning_lib.bin_cellgraph(cellgraph,binsize)
    y = 0
    for perpendicular in binned:
        __analyse_bin(cellgraph,binned,y,perpendicular,False,communication_threshold,dimension)
        y += 1
    cellgraph.set_graph_mode("Waxmann")
    
def initialize_extended_waxmann_graph(cellgraph, communication_threshold,dimension):
    binsize = int(round(1.5*communication_threshold))
    binned = binning_lib.bin_cellgraph(cellgraph,binsize)
    y = 0
    for perpendicular in binned:
        __analyse_bin(cellgraph,binned,y,perpendicular,True,communication_threshold,dimension)
        y += 1
